...
export function OpportunityDetailsModal({
	...
	return (
		<Dialog open={show} onOpenChange={onClose}>
			<DialogContent
				className='min-w-[960px] min-h-[600px] max-w-5xl max-h-[90vh] overflow-y-auto'
					...
					<div className='space-y-6'>
						...
						<div className='flex items-center gap-4 p-4 bg-gray-50 rounded-lg'>
							<Avatar className='h-16 w-16'>
								<AvatarImage
									src={`https://github.com/${item.username.toLowerCase().replace(' ', '')}.png`}
								/>
							...
							<div className='flex-1'>
								...
								<div className='flex items-center gap-2 text-sm text-gray-500 mt-1'>
									<Mail size={14} />
									<span>
										{item.username.toLowerCase().replace(' ', '.')}@email.com
									</span>
								</div>
							</div>
						</div>
						...
						<div>
							...
							<div className='bg-gray-50 border rounded-lg p-4'>
								...
								<p className='text-sm text-gray-700'>
									Olá {item.username}, gostaria de agendar uma reunião para
									discutir a proposta?
								</p>
							</div>
						</div>
